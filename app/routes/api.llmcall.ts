...
import { getProviderSettingsFromCookie, serializeApiKeysCookie } from '~/lib/api/cookies';
  ...
  if (streamOutput) {
    try {
      ...
      if (needsCookieUpdate) {
        responseHeaders.append('Set-Cookie', serializeApiKeysCookie(apiKeys, { context }));
      }
    ...
    try {
      ...
      if (!modelDetails) {
        const providerModels = models.filter((m: ModelInfo) => m.provider === provider.name);
        modelDetails =
          providerModels.find(
            (m: ModelInfo) =>
              (model.includes('sonnet') && m.name.includes('sonnet')) ||
              (model.includes('gpt-4o') && m.name.includes('gpt-4o')) ||
              (model.includes('deepseek') && m.name.includes('deepseek')),
          ) ||
          providerModels[0] ||
          models[0];
