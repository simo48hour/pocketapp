...
import bundledTemplates from '~/utils/bundledTemplates.json';
...
export async function loader({ request, context }: { request: Request; context: any }) {
  ...
  // 1. Check bundled templates first (instant, 100% reliable, zero GitHub API calls)
  const bundled = (bundledTemplates as Record<string, any[]>)[repo];
  if (bundled && bundled.length > 0) {
    return json(bundled);
  }

  // 2. Check in-memory cache next
  if (templateCache.has(repo)) {
    return json(templateCache.get(repo));
  }
  ...
  try {
    ...
    templateCache.set(repo, filteredFiles);
