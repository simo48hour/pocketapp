...
import { type ChatHistoryItem, isInternalId } from '~/lib/persistence';
...
export function HistoryItem({
  ...
  const targetUrlId = isInternalId(item.urlId) ? item.id : (item.urlId || item.id);
  const isActiveChat = urlId === targetUrlId || urlId === item.urlId || urlId === item.id;
  ...
  const handleLinkClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      // Use the anchor's document navigation: chat hooks and WebContainer stores
      // retain project state across client-side route changes. A new document
      // restores the selected project with a fresh workspace and runtime.
    },
    [selectionMode, handleItemClick],
  );
        ...
        >
          ...
          >
            <div className="flex items-center gap-2.5 text-gray-400 dark:text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              {onMoveToProject && (
                <ChatActionButton
                  toolTipContent="Move to Project"
                  icon="i-ph:folder-plus h-4 w-4"
                  onClick={(event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    onMoveToProject(item);
                  }}
                />
              )}
              <ChatActionButton
