...
const MAX_CACHED_EDITOR_STATES = 8;

function setCachedEditorState(map: Map<string, EditorState>, filePath: string, state: EditorState) {
  if (map.has(filePath)) {
    map.delete(filePath);
  } else if (map.size >= MAX_CACHED_EDITOR_STATES) {
...
function getCachedEditorState(map: Map<string, EditorState>, filePath: string): EditorState | undefined {
...
export const CodeMirrorEditor = memo(
  ({
        ...
        dispatchTransactions(transactions) {
          ...
          if (docRef.current && (transactions.some((transaction) => transaction.docChanged) || selectionChanged)) {
            ...
            if (editorStatesRef.current && docRef.current?.filePath) {
              setCachedEditorState(editorStatesRef.current, docRef.current.filePath, view.state);
            }
          }
        },
      });
    ...
    useEffect(() => {
      ...
      let state = getCachedEditorState(editorStates, doc.filePath);

      if (!state || state.doc.toString() !== doc.value) {
        state = newEditorState(doc.value, theme, settings, onScrollRef, debounceScroll, onSaveRef, [
          languageCompartment.of([]),
          envMaskingCompartment.of([createEnvMaskingExtension(() => docRef.current?.filePath)]),
        ]);

        setCachedEditorState(editorStates, doc.filePath, state);
      }
