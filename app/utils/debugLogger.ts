...
class DebugLogger {
  ...
  captureLog(level: LogEntry['level'], scope?: string, args: any[] = []): void {
    ...
    try {
      const entry: LogEntry = {
        ...
        message: this._formatMessage(args.slice(0, 10)).slice(0, 4096),

        // Never retain live objects (which can include entire file trees or DOMs).
        data: undefined,
      };
  ...
  captureTerminalLog(entry: TerminalEntry): void {
    try {
      entry = { ...entry, content: entry.content.slice(-4096) };

      // Flush even during continuous output.
      if (this._config.debounceTerminal > 0) {
        this._terminalLogQueue.push(entry);

        if (this._terminalLogQueue.length > 250) {
          this._terminalLogQueue.shift();
        }
...
export async function downloadDebugLog(filename?: string): Promise<void> {
  try {
    ...
    link.href = url;
    link.download = filename || `pocketapp-debug-${new Date().toISOString().split('T')[0]}.txt`;
