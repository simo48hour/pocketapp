...
describe('StreamingMessageParser', () => {
  ...
  describe('valid artifacts with actions', () => {
    it('removes accidental source wrappers from generated code files', () => {
      const onActionClose = vi.fn();
      const parser = new StreamingMessageParser({ callbacks: { onActionClose } });

      parser.parse(
        'source-wrapper',
        '<boltArtifact title="App" id="artifact_1"><boltAction type="file" filePath="src/App.tsx"><source>export default function App() { return <div />; }</source></boltAction></boltArtifact>',
      );
    ...
    it('removes embedded source wrappers when no media element is present', () => {
      const onActionClose = vi.fn();
      const parser = new StreamingMessageParser({ callbacks: { onActionClose } });

      parser.parse(
        'embedded-source-wrapper',
        '<boltArtifact title="App" id="artifact_1"><boltAction type="file" filePath="src/App.tsx">export default () => (<div><source>ignored wrapper</source></div>)</boltAction></boltArtifact>',
      );

      expect(onActionClose.mock.calls[0][0].action.content).not.toContain('</source>');
    });

    it('truncates a file before a leaked following artifact action', () => {
      const onActionClose = vi.fn();
      const parser = new StreamingMessageParser({ callbacks: { onActionClose } });

      parser.parse(
        'leaked-action',
        '<boltArtifact title="App" id="artifact_1"><boltAction type="file" filePath="src/App.tsx">export default App\n<boltAction type="file" filePath="src/index.css">@tailwind base;</boltAction></boltArtifact>',
      );

      expect(onActionClose.mock.calls[0][0].action.content).toBe('export default App\n');
    });
