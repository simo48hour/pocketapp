...
import { boundOutput } from '~/utils/terminal-output';
...
import { ensureBaseDependencies, sanitizeCodeImports, sanitizeGeneratedSource, sanitizeViteConfig } from '~/utils/fileUtils';
import { isDevServerRunning } from '~/lib/webcontainer/processes';
import { optimizeShellCommand } from './command-optimizer';
import { hydrationPromise } from '~/lib/webcontainer';
...
export class ActionRunner {
  ...
  onFileAction?: (filePath: string, content: string) => void;
  ...
  addAction(data: ActionCallbackData, isReloaded: boolean = false) {
    const { actionId } = data;
    ...
    this.actions.setKey(actionId, {
      ...data.action,
      status: isReloaded ? 'complete' : 'pending',
      executed: isReloaded,
      abort: () => {
  ...
  async runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    // Publish complete files only; partial content stays in the editor.
    if (isStreaming) {
      return;
    }
  ...
  async #executeAction(actionId: string, isStreaming: boolean = false) {
    ...
    try {
      ...
      const updates: ActionStateUpdate = {
        status: finalStatus,
      };

      if (action.type === 'file' && finalStatus === 'complete') {
        updates.content = '';
      }

      this.#updateAction(actionId, updates);
    } catch (error) {
      if (action.abortSignal.aborted) {
        return;
      }
  ...
  async #runShellAction(action: ActionState) {
    ...
    // Wait for prebuilt dependencies to finish extracting before checking node_modules.
    // This prevents a race where npm install runs concurrently with extract.cjs,
    // which pegs the Wasm CPU at 100% and causes browser "Page Unresponsive" crashes.
    try {
      await Promise.race([
        hydrationPromise,
      ...
      // Hydration may have failed; proceed with the install attempt.
    }
    ...
    const optimized = optimizeShellCommand(action.content, {
      nodeModulesReady,
      devServerRunning: isDevServerRunning(),
    });

    if (optimized.kind === 'skip') {
      logger.debug(`Skipped redundant command: ${action.content}`);
      return;
    }

    if (optimized.kind === 'start' && optimized.command) {
      await this.#runStartAction({ ...action, type: 'start', content: optimized.command } as ActionState);
      return;
    }

    if (optimized.command) {
      action.content = optimized.command;
    }
  ...
  async #runStartAction(action: ActionState) {
    ...
    const startCommand = action.content.trim();

    const resp = await shell.startCommand(this.runnerId.get(), startCommand, () => {
      logger.debug(`[${action.type}]:Aborting Action\n\n`, action);
      action.abort();
    });
    logger.debug(`${action.type} Shell Response: [exit code:${resp?.exitCode}]`);
  ...
  async #runFileAction(action: ActionState) {
    ...
    const webcontainer = await this.#webcontainer;
    const relativePath = action.filePath.startsWith('/')
    ...
    try {
      ...
      if (relativePath === 'package.json' || relativePath.endsWith('/package.json')) {
        contentToWrite = ensureBaseDependencies(contentToWrite);
        action.content = contentToWrite;
      } else {
        contentToWrite = repairLegacyPocketBaseClient(
          relativePath,
          sanitizeGeneratedSource(relativePath, sanitizeCodeImports(relativePath, sanitizeViteConfig(relativePath, contentToWrite))),
        );
        action.content = contentToWrite;
      }
      ...
      try {
        previous = await webcontainer.fs.readFile(relativePath, 'utf8');
      } catch {
        /* New file. */
      }

      if (previous !== contentToWrite) {
        await webcontainer.fs.writeFile(relativePath, contentToWrite);
      }

      this.onFileAction?.(relativePath, contentToWrite);
  ...
  async #runBuildAction(action: ActionState) {
    ...
    const buildProcess = await spawnTracked(webcontainer, 'npm', ['run', 'build']);

    let output = '';
    const outputPromise = buildProcess.output.pipeTo(
      new WritableStream({
        write(data) {
          output = boundOutput(output + data);
        },
      }),
    );
