...
export const LOCKED_FILES_KEY = 'pocketapp.lockedFiles';
const LEGACY_LOCKED_FILES_KEY = 'bolt.lockedFiles';
...
function initializeCache(): LockedItem[] {
  ...
  try {
    if (typeof localStorage !== 'undefined') {
      const lockedItemsJson = localStorage.getItem(LOCKED_FILES_KEY) || localStorage.getItem(LEGACY_LOCKED_FILES_KEY);
...
if (typeof window !== 'undefined') {
  window.addEventListener('storage', (event) => {
    if (event.key === LOCKED_FILES_KEY || event.key === LEGACY_LOCKED_FILES_KEY) {
