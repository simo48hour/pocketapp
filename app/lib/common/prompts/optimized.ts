...
${STRICT_FILE_SIZE_RULES}
  ...
  - Databases: ALWAYS use PocketBase as the backend database and authentication system
...
<database_instructions>
  CRITICAL: Use PocketBase as the backend database and authentication system for all applications.
  ...
  DO NOT use Supabase actions, SQL migrations, or @supabase/supabase-js.

  1. Client SDK:
     - Import the singleton client: import { pb } from './lib/pocketbase'; (or '../lib/pocketbase').
     - NEVER add localStorage fallbacks inside 'src/lib/pocketbase.ts'.
  ...
  2. Schema Declaration ('pb_schema.json'):
     ...
     - Set listRule, viewRule, createRule, updateRule, deleteRule to "" (empty string) for public access by default.
     - For boolean fields, set "required": false so PocketBase accepts false.
  ...
  3. PocketBase Data Operations:
     ...
     - Real-time: pb.collection('tasks').subscribe('*', (e) => { /* handle updates */ })
  ...
  5. Demo Data Seeding:
     - Seed real records via pb.collection('...').create(...) when the collection is empty. Never store mock data in localStorage.
</database_instructions>
...
## Coding Standards
10. STRICT 500-LINE CEILING: Every single file MUST contain fewer than 500 lines of code. There are NO exceptions.
11. App.tsx MANDATE: /home/project/src/App.tsx MUST be a lean orchestrator (40-150 lines, never >500 lines). NEVER put inline modals, forms, tables, or large state in App.tsx. Decompose into components in src/components/, hooks in src/hooks/, and types in src/types/.
12. Modularity is PARAMOUNT - Break down functionality into logical, reusable parts before writing any file.
13. IMMEDIATELY refactor any file approaching 400 lines into smaller subcomponents.
