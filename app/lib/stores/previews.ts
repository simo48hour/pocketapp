...
export class PreviewsStore {
  ...
  constructor(webcontainerPromise: Promise<WebContainer>) {
    ...
    // Override localStorage setItem to catch all changes (debounced to avoid main-thread flooding)
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem = (...args) => {
        ...
        syncTimer = setTimeout(() => this._broadcastStorageSync(), 500);
      };
    }
  }
  ...
  initWithWebContainer(webcontainer: WebContainer) {
    ...
    webcontainer.on('server-ready', (port, url) => {
      ...
      const updatedInfo: PreviewInfo = {
        port,
        ready: true,
        baseUrl: url,
      };
      this.#availablePreviews.set(port, updatedInfo);
      const existing = this.previews.get().filter((p) => p.port !== port);
      this.previews.set([...existing, updatedInfo]);
