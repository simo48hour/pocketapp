...
export class FilesStore {
  ...
  #watcherPaused = false;
  #queuedEvents: Array<[events: PathWatcherEvent[]]> = [];
  ...
  resumeWatcher() {
    ...
    if (this.#queuedEvents.length > 0) {
      ...
      this.#queuedEvents = [];
      this.#processEventBuffer(queued);
    }
  }
  ...
  constructor(webcontainerPromise: Promise<WebContainer>) {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        const deletedPathsJson =
          localStorage.getItem('pocketapp-deleted-paths') || localStorage.getItem('bolt-deleted-paths');
    ...
    if (typeof window !== 'undefined') {
      ...
      window.addEventListener('popstate', checkChatIdChange);
      window.addEventListener('pocketapp-deployed', checkChatIdChange);

      // Load locked files synchronously from localStorage without touching WebContainer
      const currentChatId = getCurrentChatId();
  ...
  async saveFile(filePath: string, content: string) {
    ...
    try {
      ...
      const relativePath = path.relative(webcontainer.workdir, fullPath);

      if (!relativePath) {
        throw new Error(`EINVAL: invalid file path, write '${relativePath}'`);
      }

      const existingFile = this.getFile(fullPath) || this.getFile(filePath);
      ...
      if (!this.#modifiedFiles.has(fullPath)) {
        this.#modifiedFiles.set(fullPath, oldContent);
      }

      // Get the current lock state before updating
      const currentFile = this.files.get()[fullPath] || this.files.get()[filePath];
      ...
      if (filePath !== fullPath && this.files.get()[filePath]) {
  ...
  initWithWebContainer(webcontainer: WebContainer) {
    if (this.#initialized) {
      return;
    }
    ...
    webcontainer.internal.watchPaths(
      {
        ...
        exclude: [
          ...
          '**/package-lock.json',
          '**/pnpm-lock.yaml',
          '**/.pocketapp-dependencies.tar.gz',
          '**/.pocketapp-extract.cjs',
        ],
        includeContent: true,
      },
  ...
  #processEventBuffer(events: Array<[events: PathWatcherEvent[]]>) {
    ...
    // Clone and notify once per batch, instead of once per filesystem event.
    const files = { ...this.files.get() };
    let changed = false;
    const update = (filePath: string, entry: Dirent | undefined) => {
      if (files[filePath] === entry) return;
    ...
    for (const { type, path, buffer } of watchEvents) {
      ...
      switch (type) {
        case 'add_dir': {
          ...
          if (!files[sanitizedPath]) update(sanitizedPath, { type: 'folder' });
          break;
        }
        case 'remove_dir': {
          update(sanitizedPath, undefined);

          for (const direntPath of Object.keys(files)) {
            if (direntPath.startsWith(sanitizedPath + '/')) {
              update(direntPath, undefined);
            }
          }
        ...
        case 'add_file':
        case 'change': {
          const previous = files[sanitizedPath];

          // Metadata-only events must not erase content written by the runner.
          if (!buffer) break;
          ...
          if (previous?.type === 'file' && previous.content === content && previous.isBinary === isBinary) break;
          update(sanitizedPath, { ...previous, type: 'file', content, isBinary });
    ...
    if (changed) {
      this.#size = Object.values(files).filter((entry) => entry?.type === 'file').length;
  ...
  #persistDeletedPaths() {
    try {
      if (typeof localStorage !== 'undefined') {
        localStorage.setItem('pocketapp-deleted-paths', JSON.stringify([...this.#deletedPaths]));
      }
    } catch (error) {
