import { prepareDeferredCommands, reusableCommand } from '~/lib/runtime/deferred-commands';
...
import { description, chatId, db, setSnapshot, getSnapshot } from '~/lib/persistence';
...
export type WorkbenchViewType = 'code' | 'diff' | 'preview' | 'database' | 'analytics';
...
export class WorkbenchStore {
  ...
  latestTerminalError: WritableAtom<{ summary: string; content: string } | undefined> =
    import.meta.hot?.data.latestTerminalError ?? atom<{ summary: string; content: string } | undefined>(undefined);
  #lastTerminalError: string = '';
  sendChatMessage?: (message: string) => void;
  modifiedFiles = new Set<string>();
  artifactIdList: string[] = [];
  #globalExecutionQueue = Promise.resolve();
  #failedActionMessages = new Set<string>();
  constructor() {
    if (import.meta.hot) {
  ...
  addToExecutionQueue(callback: () => Promise<void>) {
    this.#globalExecutionQueue = this.#globalExecutionQueue
      .catch((err) => {
        console.error('[Workbench] Execution queue error:', err);
      })
      .then(() => callback())
      .catch((err) => {
        console.error('[Workbench] Execution queue error:', err);
  ...
  initStoresWithWebContainer(webcontainer: WebContainer) {
    this.#filesStore.initWithWebContainer(webcontainer);
    this.#previewsStore.initWithWebContainer(webcontainer);
  }
  ...
  setTerminalError(content: string, summary: string) {
    if (this.#lastTerminalError === content) {
      return;
    }

    this.#lastTerminalError = content;
    this.latestTerminalError.set({ summary, content });

    this.actionAlert.set({
      type: 'error',
      title: 'Terminal / Build Error',
      description: summary,
      content,
      source: 'terminal',
    });
  ...
  fixTerminalError(errorSnippet?: string) {
    const content = errorSnippet || this.latestTerminalError.get()?.content;

    if (!content) {
      return;
    }

    const prompt = `Fix this terminal error:\n\`\`\`sh\n${content}\n\`\`\`\nPlease inspect the affected files, identify what caused this error, and provide the corrected code to fix it.`;
  ...
  get restartingEnvironment() {
    return this.#terminalStore.restarting;
  }

  restartEnvironment() {
    return this.#terminalStore.restartEnvironment();
  }
  ...
  pauseStreamsAndWatchers() {
    this.#filesStore.pauseWatcher();
    this.#terminalStore.pauseTerminalStreams();
  }

  resumeStreamsAndWatchers() {
    this.#filesStore.resumeWatcher();
    this.#terminalStore.resumeTerminalStreams();
  }

  async suspendDevServer() {
    await this.#terminalStore.suspendDevServer();
  }

  async resumeDevServer(command?: string) {
    return this.#terminalStore.resumeDevServer(command);
  }

  disposeAll() {
    this.#filesStore.clearBuffers();
  ...
  setDocuments(files: FileMap) {
    ...
    if (this.currentDocument.get() === undefined) {
      ...
      if (fileEntries.length > 0) {
        // Prefer selecting App.tsx or main entrypoint file
        const priorityOrder = [
          'src/App.tsx',
          'src/App.jsx',
          'App.tsx',
        ...
        for (const candidate of priorityOrder) {
          const match = fileEntries.find(([p]) => p.endsWith(candidate));
  ...
  #persistSnapshotTimeout: any = null;
  ...
  #persistCurrentSnapshot() {
    ...
    if (this.#persistSnapshotTimeout) {
      clearTimeout(this.#persistSnapshotTimeout);
    }

    this.#persistSnapshotTimeout = setTimeout(async () => {
      try {
        const id = chatId.get();
        ...
        const existingSnapshot = await getSnapshot(db, id).catch(() => undefined);
        await setSnapshot(db, id, {
          chatIndex: existingSnapshot?.chatIndex || '',
          files: currentFiles,
          summary: existingSnapshot?.summary,
        });
      } catch (err) {
        console.warn('Failed to auto-persist snapshot on file save:', err);
      }
    }, 500);
  }
          ...
          if (action.status === 'running' || action.status === 'pending') {
            if (action.type === 'file') {
              try {
                ...
                const fullPath = path.join(wc.workdir, action.filePath);
                let doc = this.#editorStore.documents.get()[fullPath];

                if (!doc && action.content) {
                  this.#editorStore.updateFile(fullPath, action.content);
                  doc = this.#editorStore.documents.get()[fullPath];
                }
                ...
                artifact.runner.actions.setKey(actionId, {
                  ...action,
                  content: doc?.value ?? action.content,
                  status: 'complete',
                  executed: true,
                });
              } catch (err) {
                console.warn('[Workbench] Failed to finalize stuck file action:', err);
                artifact.runner.actions.setKey(actionId, {
                  ...action,
                  status: 'complete',
                  executed: true,
  ...
  async _addAction(data: ActionCallbackData) {
    ...
    return artifact.runner.addAction(data, isReloaded);
  }

  runDeferredActions(
    actions: ActionCallbackData[],
    isCurrent: () => boolean,
    affectedMessages = actions.map((data) => data.messageId),
  ) {
    const planned = prepareDeferredCommands(actions);

    for (const data of planned) {
      this.addAction(data);
    }
    this.addToExecutionQueue(async () => {
      ...
      for (const data of planned) {
        ...
        if (affectedMessages.some((messageId) => this.#failedActionMessages.has(messageId))) {
          for (const queued of planned) {
            const state = this.#getArtifact(queued.artifactId)?.runner.actions.get()[queued.actionId];
        ...
        const key = `${data.action.type}:${data.action.content.trim()}`;

        if (reusableCommand(data) && completed.has(key)) {
          const runner = this.#getArtifact(data.artifactId)?.runner;
          const state = runner?.actions.get()[data.actionId];

          if (runner && state) {
            runner.actions.setKey(data.actionId, { ...state, status: 'complete', executed: true });
          }

          continue;
        }

        await this._runAction(data);
        completed.add(key);
      }
    });
  }
  ...
  runAction(data: ActionCallbackData, isStreaming: boolean = false) {
    ...
    if (isStreaming) {
      return; // Apply only complete files, once, through the global queue.
    } else {
  ...
  async _runAction(data: ActionCallbackData) {
    ...
    if (!action || action.executed || action.abortSignal.aborted) {
      return;
    }
    ...
    if (data.action.type === 'file') {
      ...
      // Let input/paint run between files instead of chaining microtasks indefinitely.
      await new Promise<void>((resolve) => setTimeout(resolve, 16));
    } else {
      await artifact.runner.runAction(data);
    }

    if (artifact.runner.actions.get()[data.actionId]?.status === 'failed') {
      this.#failedActionMessages.add(messageId);
    }
  }
...
if (typeof window !== 'undefined') {
  (window as any).workbenchStore = workbenchStore;
}
