(function() {
  ...
  // Preview DOM readiness notification for parent window
  function isAppRendered() {
    ...
    if (document.body) {
      ...
      for (let i = 0; i < children.length; i++) {
        ...
        if (tag !== 'script' && tag !== 'style' && tag !== 'link' && !el.id?.startsWith('vite-')) {
  ...
  let previewReadyNotified = false;
  function notifyPreviewReady() {
    if (previewReadyNotified) return;
    previewReadyNotified = true;
    try {
      window.parent.postMessage({
        type: 'PREVIEW_DOM_READY',
  ...
  if (isAppRendered()) {
    ...
    // Secondary poll every 200ms to catch fast renders
    ...
    // Generous safety fallback (20s) - prevent premature hiding before React/Vite compiles
    setTimeout(function() {
      if (pollInterval) clearInterval(pollInterval);
      notifyPreviewReady();
    }, 20000);
  ...
  window.addEventListener('beforeunload', function() {
    try {
      window.parent.postMessage({ type: 'PREVIEW_UNLOADED' }, '*');
    } catch (e) {}
  });
})();